...
import { CircleAlertIcon, CheckCircle, Tag, BadgeDollarSign } from 'lucide-react'; 
...
export default function NovoProdutoPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await postProduct({ name, price: parseFloat(price) });
      ...
      <div className="bg-white rounded-lg shadow-md p-8">
        <form onSubmit={handleSubmit}>
          ...
          <div className="mb-5 relative">
            ...
            <Tag className="absolute left-3 top-1/2 mt-2 text-gray-400" size={18} />
            <input 
              type="text"
              id="name"
              value={name}
          ...
          {/* Campo Email */}
          <div className="mb-5 relative">
            <label htmlFor="price" className="text-sm font-medium text-gray-600 mb-1 block">
              Preço
            </label>
            <BadgeDollarSign className="absolute left-3 top-1/2 mt-2 text-gray-400" size={18} />
            <input 
              type="price"
              id="price"
              value={price}
